...
@foreach ($etapas as $posicao=>$etapa)
	@if($etapa->id === 1)
		<fieldset>
			...
			<a class="btn btn-secondary" role="button" href="{{route('finish',['id'=>$idp])}}">Finalizar</a>
			@else
	...
	@if($etapa->id !=1)
		<fieldset>
			...
			@if($etapa->verificacao == 1)
				<p class="lead"><i class="fa fa-dot-circle-o icor" aria-hidden="true"></i><b>Etapa precisa de verificação pelo administrador do pocesso!</b></p>
			@endif
			</div>
