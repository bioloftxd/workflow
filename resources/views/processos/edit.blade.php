...
@section('content')
    ...
    <div class="container">
        <form method="POST" id="alterar-form" action="{{route('processos.update',["id"=>$processo->id])}}">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-lg-8">
                    ...
                    <input type="text" class="form-control" id="nome" name="nome" placeholder="Nome processo"
                           required=""
                           value="{{$processo->nome}}">
                </div>
            </div>
        ...
        <form id="desativar-form" action="{{ route('processos.destroy',['id'=>$processo->id]) }}" method="POST"
              style="display: none;">
            {{ csrf_field() }}
            {{method_field("DELETE")}}
        </form>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="float:right">
                                <a class="btn btn-secondary btn-sm" href="{{ route('etapas.edit',['id'=>$etapa->id]) }}"
                                   role="button">Editar</a>

                                <a class="btn btn-danger btn-sm" href="{{ route('etapas.destroy',['id'=>$etapa->id]) }}"
                                   onclick="event.preventDefault();
                                           document.getElementById('desativar-form{{$etapa->id}}').submit();">Desativar</a>
                                <form id="desativar-form{{$etapa->id}}"
                                      action="{{ route('etapas.destroy',['id'=>$etapa->id]) }}" method="POST"
                                      style="display: none;">
                                    {{ csrf_field()}}
                                    {{method_field("DELETE")}}
                                </form>
